...
// Live state of a join (concatenate) run (issue #163), polled by the Join page.
// One run at a time — mirrors the other producer-job progress singletons.
public sealed class JoinProgress
{
    ...
    private string _phase = "idle";   // idle | joining | stopping | done | error
