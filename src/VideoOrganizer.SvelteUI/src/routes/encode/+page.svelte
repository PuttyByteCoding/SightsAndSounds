<script lang="ts">
  // Encode / Convert page (issue #164). Search to add videos, then re-encode
  // them to the server's configured profile (ffmpeg args or a HandBrake preset
  // — see the "Encode" config section). Each produces a "<stem>_encoded.mp4"
  // ingested as a new video. Encode failures are surfaced per video.
  import { onMount, onDestroy } from 'svelte';
  import { api } from '$lib/api';
  import type { EncodeProgress, SearchResult } from '$lib/types';
  ...
  const encoding = $derived(!!progress?.active);
  ...
  onMount(async () => {
    try {
      const p = await api.getEncodeProgress();
  ...
  async function encode() {
    if (busy || encoding || picked.length === 0) return;
    busy = true; error = null;
    try {
      progress = await api.startEncode(picked.map(p => p.id));
      startPolling();
    } catch (e) { error = e instanceof Error ? e.message : String(e); }
    finally { busy = false; }
  }
  async function stop() {
    try { progress = await api.stopEncode(); } catch { /* poll reconciles */ }
  }
</script>

<div class="p-4 max-w-5xl mx-auto flex flex-col gap-4">
  <h1 class="text-2xl font-semibold">Encode / Convert</h1>
  <p class="text-sm text-base-content/60">
    Re-encode videos to the server's configured profile (ffmpeg args or a
    HandBrake preset — set the <code>Encode</code> config section). Each produces a
    new <code>&lt;name&gt;_encoded.mp4</code>; failures are listed per video.
  </p>
  ...
  {#if progress && (progress.active || progress.phase !== 'idle')}
    <div class="alert {progress.phase === 'error' ? 'alert-warning' : 'alert-info'} text-sm flex-col items-start gap-2">
      <div class="flex items-center gap-2 w-full">
        ...
        <span>{progress.active ? 'Encoding' : progress.phase === 'error' ? 'Finished with errors' : 'Encoded'} — {progress.done}/{progress.total}</span>
  ...
  <div class="grid gap-4 md:grid-cols-2">
    ...
    <div class="flex flex-col gap-2">
      <div class="text-sm font-medium">To encode ({picked.length})</div>
      ...
      <ul class="flex flex-col gap-1">
        {#each picked as p (p.id)}
          <li class="flex items-center gap-2 p-1 rounded bg-base-200">
            ...
            <button class="btn btn-ghost btn-xs" onclick={() => remove(p.id)} disabled={encoding} title="Remove">✕</button>
          </li>
        {/each}
      </ul>
      <div class="flex gap-2 mt-2">
        ...
        {:else}
          <button class="btn btn-sm btn-primary" onclick={encode} disabled={busy || picked.length === 0}>
            {#if busy}<span class="loading loading-spinner loading-xs"></span>{/if}
            Encode {picked.length} video{picked.length === 1 ? '' : 's'}
          </button>
        {/if}
      </div>
    </div>
